...
#define SAFNUM_BGP_LS_SPF      80  /* draft-ietf-lsvr-bgp-spf-29 */
...
#define BGP_NLRI_TLV_ADDRESS_FAMILY                 266
    ...
    while (length > 0) {
        ...
        switch (type) {
            ...
            case BGP_NLRI_TLV_ADDRESS_FAMILY:
                if(sub_length != BGP_NLRI_TLV_LEN_ADDRESS_FAMILY){
                    expert_add_info_format(pinfo, tlv_tree, &ei_bgp_ls_error,
                                           "Unexpected Address Family TLV's length (%u), it must be %u bytes!",
                                           sub_length, BGP_NLRI_TLV_LEN_ADDRESS_FAMILY);
                    return -1;
                }
                tlv_sub_item = proto_tree_add_item(tlv_tree,
                                                   hf_bgp_ls_tlv_address_family, tvb, offset,
                                                   sub_length + 4, ENC_NA);
                tlv_sub_tree = proto_item_add_subtree(tlv_sub_item, ett_bgp_mp_reach_nlri);
            break;
...
decode_link_state_attribute_tlv(proto_tree *tree, tvbuff_t *tvb, gint offset, packet_info *pinfo, guint8 protocol_id)
{
    ...
    switch (type) {
        ...
        case BGP_NLRI_TLV_SPF_STATUS:
            ...
            proto_tree_add_item(tlv_tree, hf_bgp_ls_type, tvb, offset, 2, ENC_BIG_ENDIAN);
            proto_tree_add_item(tlv_tree, hf_bgp_ls_length, tvb, offset + 2, 2, ENC_BIG_ENDIAN);
            proto_tree_add_item(tlv_tree, hf_bgp_ls_tlv_spf_status_value, tvb, offset + 4, 1, ENC_BIG_ENDIAN);
            break;
                 ...
                 const char *tag, packet_info *pinfo)
{
    ...
    case AFNUM_BGP_LS:
        ...
        if (safi == SAFNUM_BGP_LS || safi == SAFNUM_BGP_LS_VPN || safi == SAFNUM_BGP_LS_SPF) {
